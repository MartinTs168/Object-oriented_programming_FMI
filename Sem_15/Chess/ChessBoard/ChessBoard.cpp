...
ChessBoard::ChessBoard()
{
	...
	board[0][3].figure = factory.createFigure(false, Figure::FigureType::QueenFigure);
	board[7][3].figure = factory.createFigure(true, Figure::FigureType::QueenFigure);

	board[0][4].figure = factory.createFigure(false, Figure::FigureType::KingFigure);
	board[7][4].figure = factory.createFigure(true, Figure::FigureType::KingFigure);
}
...
bool ChessBoard::isPlayerInCheck(bool isWhite) const
{
	...
	for (size_t i = 0; i < BOARD_SIZE; i++)
	{
		for (size_t j = 0; j < BOARD_SIZE; j++)
		{
			...
			{
				if (isMovable(i, j, kingX, kingY, board[i][j].figure->getIsWhite()))
				{
					return true;
				}
			}
